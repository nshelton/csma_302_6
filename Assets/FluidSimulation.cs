...
public class FluidSimulation : MonoBehaviour
{
    ...
    [SerializeField, Range(0, 30)] public float _velocityScale;
    ...
    // underscores before function names distinguish helper functions from functions
    void _DispatchKernal(string name,
    ...
    void Update() {
       ...
       if (Input.GetMouseButton(0)){
            ...
            _DispatchKernal("Force", destinationFluid: _fluidA);
        }

        _lastMousePos = Vector3.Lerp(_lastMousePos, Input.mousePosition, 0.5f);

        _DispatchKernal("BoundaryVelocity", sourceFluid: _fluidA, destinationFluid: _fluidA);
        _DispatchKernal("Advection", sourceFluid: _fluidA, destinationFluid: _fluidB);

        for(int i = 0; i < _diffuseIterations; i ++) {
            _DispatchKernal("Diffuse", sourceFluid: _fluidB, destinationFluid: _fluidA);
            _DispatchKernal("Diffuse", sourceFluid: _fluidA, destinationFluid: _fluidB);
        }

        _DispatchKernal("Divergence", sourceFluid: _fluidB, destinationDivergence: _divergence);
        _DispatchKernal("Clear", destinationFluid: _pressureA);

        for(int i = 0; i < _pressureIterations; i ++) {
            _DispatchKernal("Pressure", sourcePressure: _pressureA, sourceDivergence: _divergence, destinationPressure: _pressureB);
            _DispatchKernal("Pressure", sourcePressure: _pressureB, sourceDivergence: _divergence, destinationPressure: _pressureA);
        }

        _DispatchKernal("BoundaryPressure", sourcePressure: _pressureA, destinationPressure: _pressureA);
        _DispatchKernal("ProjectField", sourceFluid: _fluidB, destinationFluid: _fluidA, sourcePressure: _pressureA);
    }
